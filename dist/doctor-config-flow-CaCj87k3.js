...
import { d as resolveDefaultAgentId } from "./agent-scope-CUpt2978.js";
...
import { An as getTrustedSafeBinDirs, Bt as readConfigFileSnapshot, Ft as migrateLegacyConfig, Kt as OpenClawSchema, Nn as normalizeTrustedSafeBinDirs, Un as resolveCommandResolutionFromArgv, cr as resolveSlackNativeStreaming, jn as isTrustedSafeBinPath, lr as resolveSlackStreamingMode, sr as resolveDiscordPreviewStreamMode, ur as resolveTelegramPreviewStreamMode } from "./model-selection-J6oFwo9y.js";
import { t as formatCliCommand } from "./command-format-ChfKqObn.js";
import { f as parseToolsBySenderTypedKey } from "./dock-xZn_ilyf.js";
import { X as canonicalizeMainSessionAlias, p as saveSessionStore } from "./sessions-Hkcy8tM7.js";
import { d as resolveTelegramAccount, l as listTelegramAccountIds } from "./plugins-D13me3z9.js";
...
import { t as collectProviderDangerousNameMatchingScopes } from "./dangerous-name-matching-CTQpOYZ9.js";
import { t as applyPluginAutoEnable } from "./plugin-auto-enable-ClrgoY6j.js";
import { r as shouldMoveSingleAccountChannelKey, t as fetchTelegramChatId } from "./api-B93YbptW.js";
import { t as note } from "./note-DrnU1H_P.js";
...
function detectEmptyAllowlistPolicy(cfg) {
	...
	if (!channels || typeof channels !== "object") return [];
	const warnings = [];
	const checkAccount = (account, prefix, parent) => {
		const dmEntry = account.dm;
		const dm = dmEntry && typeof dmEntry === "object" && !Array.isArray(dmEntry) ? dmEntry : void 0;
		...
		if ((account.dmPolicy ?? dm?.policy ?? parent?.dmPolicy ?? parentDm?.policy ?? void 0) !== "allowlist") return;
		const topAllowFrom = account.allowFrom ?? parent?.allowFrom;
		const nestedAllowFrom = dm?.allowFrom;
		const parentNestedAllowFrom = parentDm?.allowFrom;
		if (hasAllowFromEntries(topAllowFrom ?? nestedAllowFrom ?? parentNestedAllowFrom)) return;
		warnings.push(`- ${prefix}.dmPolicy is "allowlist" but allowFrom is empty — all DMs will be blocked. Add sender IDs to ${prefix}.allowFrom, or run "${formatCliCommand("openclaw doctor --fix")}" to auto-migrate from pairing store when entries exist.`);
	};
